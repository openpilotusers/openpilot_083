...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    steerActuatorDelay = float(int(params.get('SteerActuatorDelayAdj')) * 0.01)
    steerRateCost = float(int(params.get('SteerRateCostAdj')) * 0.01)
    steerLimitTimer = float(int(params.get('SteerLimitTimerAdj')) * 0.01)
    steerRatio = float(int(params.get('SteerRatioAdj')) * 0.1)

    controlMethod = int(params.get('LateralControlMethod'))
    feedForward = True if params.get('ShaneFeedForward') == b'1' else False # Shane's feedforward
    ...
    if controlMethod == 0:
      ...
      ret.lateralTuning.pid.newKfTuned = feedForward
    elif controlMethod == 1:
