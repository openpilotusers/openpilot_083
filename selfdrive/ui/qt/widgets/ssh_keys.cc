...
AccMode::AccMode() : AbstractControl("크루즈 프로파일 설정", "크루즈 프로파일을 설정합니다. NORMAL/SPORT/ECO", "../assets/offroad/icon_shell.png") {
  ...
  QObject::connect(&btnminus, &QPushButton::released, [=]() {
    auto str = QString::fromStdString(Params().get("OpkrAccMode"));
    int value = str.toInt();
    value = value - 1;
    if (value <= 0 ) {
      value = 0;
    ...
    QString values = QString::number(value);
    Params().write_db_value("OpkrAccMode", values.toStdString());
    refresh();
  });
...
void AccMode::refresh() {
  QString option = QString::fromStdString(Params().get("OpkrAccMode"));
  if (option == "0") {
    label.setText(QString::fromStdString("NORMAL"));
  } else if (option == "1") {
    label.setText(QString::fromStdString("SPORT"));
  } else {
    label.setText(QString::fromStdString("ECO"));
  }
  btnminus.setText("◀");
  btnplus.setText("▶");
}
