...
void HomeWindow::mousePressEvent(QMouseEvent* e) {
  ...
  // LAT mode
  if (ui_state->scene.started && ui_state->sidebar_collapsed && !ui_state->scene.map_on_top && lat_btn.ptInRect(e->x(), e->y())) {
    ui_state->scene.lat_mode = ui_state->scene.lat_mode + 1;
    if (ui_state->scene.lat_mode > 2) {
      ui_state->scene.lat_mode = 0;
    }
    if (ui_state->scene.lat_mode == 0) {
      Params().write_db_value("OpkrLatMode", "0", 1);
    } else if (ui_state->scene.lat_mode == 1) {
      Params().write_db_value("OpkrLatMode", "1", 1);
    } else if (ui_state->scene.lat_mode == 2) {
      Params().write_db_value("OpkrLatMode", "2", 1);
    }
    return;
  }
  // ML Button
  if (ui_state->scene.started && ui_state->sidebar_collapsed && !ui_state->scene.map_on_top && ml_btn.ptInRect(e->x(), e->y())) {
    ui_state->scene.mlButtonEnabled = !ui_state->scene.mlButtonEnabled;
    return;
  }
